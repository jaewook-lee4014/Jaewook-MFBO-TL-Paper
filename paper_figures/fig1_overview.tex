...
%% (DKL; up to 45x fewer FLOPs). Edited 2026-09-03 for the NCS submission:
%% panels widened and shortened so the content fills them, fonts one step larger, included at \textwidth.
%% Counts updated 2026-09-11 to the final data basis (13 benchmarks; 3 GP + 9 TL surrogates: NARGP excluded,
%% Curriculum/KD/Pseudo-labelling merged into feature-extraction transfer) and to the Fig. 1b-p attainment result
%% (best TL at or above best GP on all 9 chemistry & materials pools).
...
\node[text=sub, font=\scriptsize, anchor=north, align=center] at (\Ac,\yb-2.12)
     {13 benchmarks: 4 synthetic $\cdot$ 9 chemistry \& materials};
...
\node[glab, anchor=west] at (12.4,3.3) {MFGP $\cdot$ DKL\\sparse variational MFGP};

% TL lane
\node[chip, fill=tlbox, anchor=west] (tlc) at (\Bl+0.35,1.95)
     {Transfer-learning\\(TL) surrogates $\times 9$};
...
\node[slab, anchor=north] at (\Cc,\yb-0.82) {on chemistry \& materials benchmarks};
...
\end{scope}
\node[text=sub, font=\footnotesize, anchor=north] at (\Cc,2.62) {TL $\geq$ GP on all 9 chemistry \& materials pools};
