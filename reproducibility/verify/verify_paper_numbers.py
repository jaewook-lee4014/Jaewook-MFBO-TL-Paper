#!/usr/bin/env python
"""verify_paper_numbers.py (2026-09-14, reproducibility reconciliation)
...
the cell-selection rule ("largest horizon wins", seeds 42-81, NARGP dropped, Curriculum = KD = PL merged) and every
aggregation from scratch; it reads none of the value CSVs except to COMPARE against them. Nothing is modified.

Run on the VM:  OUT=/mnt/data/jaewook_mfbo/verify_out python verify_paper_numbers.py
...
RENAME = {'DNGOGradient': 'End-to-End Joint', 'DNGO-Gradient': 'End-to-End Joint', 'DNGOJoint': 'Stop-Gradient Joint', 'DNGO-Joint': 'Stop-Gradient Joint',
          ...
          'KnowledgeDistillation': 'KD', 'Knowledge Distillation': 'KD', 'PseudoLabeling': 'PL', 'Pseudo-Labeling': 'PL', 'Curriculum': MERGED,
...
TL9 = ['Sequential', MERGED, 'End-to-End Joint', 'Progressive', 'Pretrain-then-Joint', 'Stop-Gradient Joint', 'Domain Adaptation (MMD)',
       'Soft Parameter Sharing', 'Adapter']
M12 = GP3 + TL9
ABBR = {'MFGP': 'MFGP', 'Sparse MFGP': 'SV-MFGP', 'DKL': 'DKL', 'Sequential': 'Seq', MERGED: 'FET', 'End-to-End Joint': 'E2E', 'Progressive': 'Prog',
        'Pretrain-then-Joint': 'PtJ', 'Stop-Gradient Joint': 'SGJ', 'Domain Adaptation (MMD)': 'MMD', 'Soft Parameter Sharing': 'SPS', 'Adapter': 'Adpt'}
...
EXPL = json.load(open(f'{R}/figcand2_20260911/explorer_data.json'))
...
CELLS, dupc = to_cells(ALL, keep_models=set(M12 + ['KD', 'PL']))
...
# KD/PL identity vs Curriculum (merged FET row)
ident = []; maxdev = 0.0
for (b, m, s), c in CELLS.items():
    if m == MERGED:
        for alt in ('KD', 'PL'):
            c2 = CELLS.get((b, alt, s))
            if c2 is not None:
                n = min(len(c['reg']), len(c2['reg'])); d = float(np.max(np.abs(c['reg'][:n] - c2['reg'][:n]))); maxdev = max(maxdev, d)
                ident.append(d <= 1e-9)
log(f'  Curriculum vs KD/PL trajectories identical: {sum(ident)}/{len(ident)} (max |diff| {maxdev:.3g})')
REPORT['fet_identity'] = dict(identical=int(sum(ident)), compared=len(ident), maxdev=maxdev)
...
ref = pd.read_csv(f'{R}/figcand2_20260911/fig1_attainment_values.csv')
...
refo = pd.read_csv(f'{R}/figcand2_20260911/fig1_attainment_summary_score.csv', index_col=0); refp = pd.read_csv(f'{R}/figcand2_20260911/fig1_attainment_summary_score_chem.csv', index_col=0)
...
log(f'  TL rows above every GP: o {REPORT["panel_o_TL_above_every_GP"]}/9, p {REPORT["panel_p_TL_above_every_GP"]}/9; TL score range o {So.loc[TL9,"score"].min():.2f}-{So.loc[TL9,"score"].max():.2f}')
...
ref2 = pd.read_csv(f'{R}/figcand2_20260911/fig2_trajectories_values.csv')
...
FAM = {'TL': ['Progressive', 'KnowledgeDistillation', 'PseudoLabeling'], 'MFGP': ['MFGP'], 'variants': ['DKL', 'SparseMFGP']}
grows = []; nseeds = set(); nhf = set(); nlf = set()
for f in sorted(glob.glob(f'{R}/figrepo/results/grid/cells/summary_cell_*.csv')):
...
GPF = {'MFGP', 'Sparse MFGP', 'DKL Multi-Fidelity'}; d['family'] = np.where(d.model.isin(GPF), 'GP', 'TL')
...
try:
    refl = json.load(open(f'{R}/figrepo/figures/out/dkl/scaling_law_blr.json')); REPORT['flops']['ref_scaling_law_json'] = refl
except Exception as e:
    log('  (no scaling_law_blr.json in out/dkl:', e, ')')
...
ref_s2 = pd.read_csv(f'{R}/figcand2_20260911/supp/supp_acq_portfolio_meandelta.csv', index_col=0)
...
init = []
for p in B9 + LARGE4:
    files = glob.glob(f'{R}/results/blr_replace__hf_argmin/cells/summary_{p}_Sequential_*.csv') or glob.glob(f'{X}/results_confirm_ext/blr_replace__hf_argmin/cells/summary_{p}_Sequential_*.csv')
