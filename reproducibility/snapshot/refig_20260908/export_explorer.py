...
for b in POOLS:
    ...
    for m in ORDER:
        ...
        for s in range(42, 82):
            ...
            rows[str(s)] = {"hz": float(hz), "pts": [[round(x, 3), float(f"{y:.6g}")] for x, y in pts]}
        if rows: P["series"][m] = rows
    data["pools"][b] = P
out = "/mnt/data/jaewook_mfbo/MFBO-TL-Paper/experiments/refig_20260908/figcand2_20260911/explorer_data.json"
