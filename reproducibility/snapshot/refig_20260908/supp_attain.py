# -*- coding: utf-8 -*-
"""Supplementary / Extended-Data figures under the paper's unified attainment metric (2026-09-11 evening).
...
Figures (paper style: 7.2 in wide, DejaVu Sans 6-7 pt, two hues TL #4e95d9 / GP #f2aa84, no bold):
  ...
  S2  supp_acq_portfolio   acquisition portfolio (greedy, EI, PI, GP-UCB, MES, Thompson; 9 TL rows; 20 seeds)
...
Outputs to OUTDIR (default figcand2_20260911/supp): png (300 dpi) + pdf + values csv + supp_summary.json.
"""
...
OUTDIR = os.environ.get("OUTDIR", f"{REFIG}/figcand2_20260911/supp"); os.makedirs(OUTDIR, exist_ok=True)
EXPL = json.load(open(f"{REFIG}/figcand2_20260911/explorer_data.json"))
...
RAW = {"DNGOGradient": "End-to-End Joint", "DNGOJoint": "Stop-Gradient Joint", "TwoStageJoint": "Pretrain-then-Joint", "DomainAdaptationMMD": "Domain Adaptation (MMD)",
       "SoftParameterSharing": "Soft Parameter Sharing", "KnowledgeDistillation": MERGED, "PseudoLabeling": MERGED, "Curriculum": MERGED,
       "Sequential": "Sequential", "Progressive": "Progressive", "Adapter": "Adapter", "MFGP": "MFGP", "SparseMFGP": "Sparse MFGP", "DKL": "DKL"}
...
def merged_rows(traj, pool, b, seeds=None, tag=""):
    """Attainment per display model (12 rows). FET = Curriculum row; checks KD/PL identity and records the max deviation."""
    rows = {}; dev = []
    def get(raw):
    ...
    for raw, disp in RAW.items():
        if disp == MERGED:
            if raw != "Curriculum":
                ps = get(raw); common = set(cur) & set(ps)
                if common: dev.append(max(abs(cur[s] - ps[s]) for s in common))
                if not cur and ps: cur = ps
            continue
        ps = get(raw)
        if ps: rows[disp] = ps
    if cur: rows[MERGED] = cur
    if dev: SUMMARY.setdefault("fet_merge_maxdev", {})[f"{tag}|{pool}"] = float(max(dev))
    return rows
...
FOOT1 = "TL: Seq = Sequential · FET = Feature-extraction transfer · E2E = End-to-End Joint · Prog = Progressive · PtJ = Pretrain-then-Joint · SGJ = Stop-Gradient Joint"
FOOT2 = "MMD = Domain Adaptation (MMD) · SPS = Soft Parameter Sharing · Adpt = Adapter   GP: MFGP = baseline MFGP · SV-MFGP = sparse variational MFGP · DKL = deep-kernel GP"
...
def fig_S1():
    ...
    # heat-map: EI - greedy per surrogate (TL 9 rows at 20 seeds 42-61; MFGP at 40 seeds), positive = EI better
    H = []; hn = []
    for disp in ["MFGP"] + TLM:
        ...
        for p in B9:
            ...
            if disp == "MFGP":
                ...
                raw = [k for k, v in RAW.items() if v == disp and k != "KnowledgeDistillation" and k != "PseudoLabeling"][0]
