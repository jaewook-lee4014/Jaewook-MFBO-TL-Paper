...
def run_one(bench, cfg, model_name, seed, device):
    ...
    while cur < budget and it < max_it:
        ...
        if WARM and model is not None and hasattr(model, 'lf_net'):
            ...
            model.fit(X[L], y_lf[L], X[Hh], y_hf[Hh])
        ...
        if eval_hf:
            ...
            nxt = int(np.argmin(mm)); hf_idx.add(nxt); cur += 1.0; lfc = 0; picks.append(float(pct[nxt]))
        else:
            ...
            nxt = int(np.argmax(acq)); lf_idx.add(nxt); cur += rho; lfc += 1
...
def main():
    ...
    args = ap.parse_args()
    import torch
    dev = torch.device('cpu'); torch.set_num_threads(int(os.environ.get('TORCH_THREADS', '1')))
    arm = arm_name()
    ...
    for bn in args.benches:
        b, cfg = POOLS.make_bench(bn, REPO / 'data')
        print(f'{bn}: N={b.n_candidates} dim={b.dim} budget={cfg["budget"]} rho={cfg["cost_ratio"]}', flush=True)
